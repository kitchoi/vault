...
(function() {
  ...
  let copyToClipboard = function() {
    let password = word.value.replace(/'/g, '\\\'');
  ...
  let copyPassword = $('copy-password');
  on(copyPassword, 'click', function(e) {
    e.preventDefault();
    copyToClipboard();
  });
  on(service, 'keydown', function(e) {
    if (e.keyCode === 16) copyToClipboard();
  });
  on(phrase, 'keydown', function(e) {
    if (e.keyCode === 16) copyToClipboard();
  });
